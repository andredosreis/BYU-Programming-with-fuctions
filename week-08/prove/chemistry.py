...
# Get a chemical formula for a molecule from the user.
    ...
    compound = parse_formula(formula, table)
    print(compound)

    molar_mass = compute_molar_mass(compound, table)

    number = mass  / molar_mass

    print(f"{molar_mass} Grams/Mole")
    print(f"{number} moles")
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for e in symbol_quantity_list:
        ...
        symbol = e[NAME_INDEX]
        quantity = e[ATOMIC_MASS_INDEX]

        atomic_mass = quantity * periodic_table_dict[symbol][1]
